...
#ifdef V6_TRD
* the XAUI driver (xaui) and the Raw Data driver (xrawdata).
#else
* the Gigabit Ethernet driver (xgbeth) and the Block Data driver 
* (xblockdata).
#endif
*
...
#ifdef S6_TRD
* <b><i> Note: In the given example user drivers, since xgbeth is an Ethernet 
* driver under the system's TCP/IP stack, it does not do any packet 
* generation within the driver. </i></b>
#endif
* 
...
typedef struct {
    ...
    int (* UserInit)(unsigned long barbase, unsigned int privdata); // guodebug unsigned int -> unsigned long
