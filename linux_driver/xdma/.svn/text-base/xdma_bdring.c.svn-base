...
int Dma_BdRingStart(Dma_BdRing * RingPtr)
{
     ...
     */
    Dma_mWriteReg(RingPtr->ChanBase, REG_DMA_ENG_NEXT_BD, (u32)RingPtr->BdaRestart); //guodebug error prone
    Dma_mWriteReg(RingPtr->ChanBase, REG_SW_NEXT_BD, (u32)RingPtr->BdaRestart); //guodebug error prone
    log_verbose("RingPtr->BdaRestart = %p\n", RingPtr->BdaRestart);
    wmb();
    ...
    if (RingPtr->HwCnt > 0) 
    {
        Dma_mWriteReg(RingPtr->ChanBase, REG_SW_NEXT_BD, (u32)Dma_mVirtToPhys(RingPtr->HwTail)); //guodebug error prone
        log_verbose("Physical Addr = %p\n", Dma_mVirtToPhys(RingPtr->HwTail)); //guodebug error prone
    }
 ...
 *
 *****************************************************************************/
int Dma_BdRingAlloc(Dma_BdRing* RingPtr, unsigned int NumBd, Dma_Bd** BdSetPtr)
{
    /* Enough free BDs available for the request? */
    if (RingPtr->FreeCnt < NumBd) 
    {
        log_verbose(KERN_INFO "Ring %p free count is only %d\n", RingPtr, (RingPtr->FreeCnt));
        return (XST_FAILURE);
    }
...
int Dma_BdRingToHw(Dma_BdRing * RingPtr, unsigned NumBd, Dma_Bd * BdSetPtr)
{
    ...
    if ((RingPtr->PreCnt < NumBd) || (RingPtr->PreHead != BdSetPtr)) {
        printk(KERN_INFO "PreCnt is %d, PreHead is %p\n", RingPtr->PreCnt, (RingPtr->PreHead));
...
int Dma_BdRingCheck(Dma_BdRing * RingPtr)
{
    ...
    AddrV = (Xaddr) RingPtr->FreeHead;
    if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
        return (XST_DMA_SG_LIST_ERROR);
    }

    AddrV = (Xaddr) RingPtr->PreHead;
    if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
        return (XST_DMA_SG_LIST_ERROR);
    }

    AddrV = (Xaddr) RingPtr->HwHead;
    if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
        return (XST_DMA_SG_LIST_ERROR);
    }

    AddrV = (Xaddr) RingPtr->HwTail;
    if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
        return (XST_DMA_SG_LIST_ERROR);
    }

    AddrV = (Xaddr) RingPtr->PostHead;
    if ((AddrV < RingPtr->FirstBdAddr) || (AddrV > RingPtr->LastBdAddr)) {
        return (XST_DMA_SG_LIST_ERROR);
    }
